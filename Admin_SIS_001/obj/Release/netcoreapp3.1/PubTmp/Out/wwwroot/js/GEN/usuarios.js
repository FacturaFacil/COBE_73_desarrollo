...
function cargarDataTable() {
    dataTable = $('#tblUsuarios').DataTable({
        "ajax": {
            "url": "/Cliente/GEN_Usuarios/GetAll",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            { "data": "id", "width": "10%" },
            {
                "width": "30%", "render": function (data, type, row) {
            ...
            { "data": "email", "width": "15%" },
            {
                "data": "activo", "width": "15%", "render": function (data, type, row) {
                    if (data == true) {
            ...
            },
            {
                "data": "id", "render": function (data) {
                    return `<div class="text-center">
                            <p class="mb-0">
                                <a href='/Cliente/GEN_Usuarios/Edit/${data}' class="btn btn-success btn-circle text-white" ><i class="fa fa-edit btnEditar"></i></a>
                                <a onclick=Delete('/Cliente/GEN_Usuarios/Delete/${data}') class="btn btn-danger btn-circle text-white"><i class="fa fa-trash btnEliminar"></i></a>
                            </p>
...
function ClicTabla() {
    $("#tblUsuarios tbody").on("click", ".cb_activo", function (e) {

        var row = $('#tblUsuarios').DataTable().row($(this).parents('tr')).data();

        $.ajax({
            //  async: false,
            type: "POST",
            url: "/Cliente/GEN_Usuarios/CambiaEstatus",
            data: { id: row.id },
            dataType: "json",
            beforeSend: function () {
            },
...
$(document).on('change', '#txtId', function () {
    ...
    $.ajax({
        //  async: false,
        method: "POST",
        url: "/Admin/GEN_Usuarios/ExisteFolio",
        ...
        success: function (data) {
            if (data.existe == true) {
                ...
                swal("Admin SIS!", "Esa clave ya existe, el sistema recalculara el consecutivo!", "warning")
            }
        }, error: function (xhr, ajaxOptions, thrownError) {
