...
$(document).on('change', '#txtId', function () {
        $.ajax({
            //  async: false,
            method: "POST",
            url: "/Cliente/INV_ClasesGastos/ExisteFolio",
...
function CargarDataTable() {
    dataTable = $('#tblClasesGastos').DataTable({
        "ajax": {
            "url": "/Cliente/INV_ClasesGastos/GetAll",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            ...
            {
                "data": "predefinido", "width": "10%", "render": function (data, type, row) {
                    if (data == true) {
                        return `<p class="m-0 text-center"><span class="badge badge-success">Si</span></p>`;
                    } else {

                    } return `<p class="m-0 text-center"><span class="badge badge-danger">No</span></p>`;
            ...
            },
            {
                "data": "id", "render": function (data) {
                    return `<div class="text-center">
                            <p class="mb-0">
                                <a href='/Cliente/INV_ClasesGastos/Edit/${data}' class="btn btn-success btn-circle text-white" ><i class="fa fa-edit btnEditar"></i></a>
                                <a onclick=Delete('/Cliente/INV_ClasesGastos/Delete/${data}') class="btn btn-danger btn-circle text-white"><i class="fa fa-trash btnEliminar"></i></a>
                            </p>
...
function ClicTabla() {
    $("#tblClasesGastos tbody").on("click", ".cb_activo", function (e) {

        var row = $('#tblClasesGastos').DataTable().row($(this).parents('tr')).data();

        $.ajax({
            //  async: false,
            type: "POST",
            url: "/Cliente/INV_ClasesGastos/CambiaEstatus",
            data: { id: row.id },
            dataType: "json",
            beforeSend: function () {
            },
