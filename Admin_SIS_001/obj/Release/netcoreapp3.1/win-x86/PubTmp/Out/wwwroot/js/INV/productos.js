...
$(document).on('change', '#txtId', function () {
        $.ajax({
            //  async: false,
            method: "POST",
            url: "/Cliente/INV_Productos/ExisteFolio",
...
$(document).on('change', '#Producto_Maneja_Iva_Tras', function () {

    if ($("#Producto_Maneja_Iva_Tras").is(':checked')) {
        agregarImpuesto("iva_tras")
    } else {
...
function agregarImpuesto(imp)
{
    var rowCount = $('#tblImpuestos tbody tr').length;

    if (imp == "iva_tras") {
        var ivaTrasTr = `<tr>
                     <td><label name="ListaImpuestos[${rowCount}].Id_Impuesto">002</label></td>
                     <td><label name="ListaImpuestos[${rowCount}].Descripcion_Impuesto">IVA</label></td>
                     <td> <select name="ListaImpuestos[${rowCount}].Tipo_Factor" class="form-control form-control-sm" id="cb_Factor">
                             <option value="Tasa">Tasa</option>
                             <option value="Exento">Exénto</option>
                          </select>
                    </td>                    
                     <td><select name="ListaImpuestos[${rowCount}].Tasa_O_Cuota" class="form-control form-control-sm" id="cb_valor">
                             <option value="16">16</option>
                             <option value="8">8</option>
                             <option value="0">0</option>
                          </select></td>
                     <td><label name="ListaImpuestos[${rowCount}].Tipo_Impuesto">T</label></td>
                     <td>
        ...
        $("#tblImpuestos tbody").append(ivaTrasTr);
    }
}
function CargarDataTable() {
    dataTable = $('#tblProductos').DataTable({
        "ajax": {
            "url": "/Cliente/INV_Productos/GetAll",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            ...
            { "data": "unidad", "width": "10%" },
            { "data": "id_marca", "width": "10%" },
            { "data": "clase", "width": "10%" },
            {
            ...
            },
            {
                "data": "id", "render": function (data) {
                    return `<div class="text-center">
                            <p class="mb-0">
                                <a href='/Cliente/INV_Productos/Edit/${data}' class="btn btn-success btn-circle text-white" ><i class="fa fa-edit btnEditar"></i></a>
                                <a onclick=Delete('/Cliente/INV_Productos/Delete/${data}') class="btn btn-danger btn-circle text-white"><i class="fa fa-trash btnEliminar"></i></a>
                            </p>
        ...
        buttons: [
            ...
            {
                extend: 'print', title: _doc, text: "IMPRIMIR",
                exportOptions: {
                    columns: [0, 1, 2]
                },
...
function ClicTabla() {
    $("#tblProductos tbody").on("click", ".cb_activo", function (e) {

        var row = $('#tblProductos').DataTable().row($(this).parents('tr')).data();

        $.ajax({
            //  async: false,
            type: "POST",
            url: "/Cliente/INV_Productos/CambiaEstatus",
            data: { id: row.id },
            dataType: "json",
            beforeSend: function () {
            },
