//import { tree } from "d3";

var dataTable;
var _doc = "Almacén(es)";
window.tippyInstances = [];
$(document).ready(function () {

    CargarDataTable();
    CargarTooltip();
    
});

function CargarTooltip() {

    new jBox('Tooltip', { attach: '#btnAdd', title: "Agregar " + _doc });
    new jBox('Tooltip', { attach: '.btnEditar', title: "Editar " + _doc });
    new jBox('Tooltip', { attach: '.btnEliminar', title: "Eliminar " + _doc });
    new jBox('Tooltip', { attach: '#btnGuardar', title: "Guardar " + _doc });
    new jBox('Tooltip', { attach: '#btnActualizar', title: "Actualizar " + _doc });
    new jBox('Tooltip', { attach: '#btnCancelar', title: "Cancelar datos de " + _doc });
    new jBox('Tooltip', { attach: '#btnRegresar', title: "Regresar a lista de " + _doc });

}

$(document).on('change', '#txtId', function () {
    if ($('#Almacen_Id_Sucursal').val() != "") {
        $.ajax({
            //  async: false,
            method: "POST",
            url: "/Cliente/INV_Almacenes/ExisteFolio",
            //contentType: "application/json; charset=utf-8",
            //data: { param: $('#txtId').val() + "|" + $('#Almacen_Id_Sucursal').val() },
            data: { param: $('#txtId').val()},
            dataType: "json",
            beforeSend: function () {
            },
            success: function (data) {
                if (data.existe == true) {
                    $('#txtId').val(""); $('#txtId').val(data.folio);
                    swal("COBE 73!", "Esa clave ya existe, el sistema recalculara el consecutivo!", "warning")
                }
            }, error: function (xhr, ajaxOptions, thrownError) {
                throw new Error('Error al cargar...');
            }, complete: function () {
            }
        });
    } else {
        toastr.error('Es necesario seleccionar una sucursal');
        $('#txtId').val("");

    }


});

//$(document).on('change', '#Almacen_Id_Sucursal', function () {
//    if ($('#Almacen_Id_Sucursal').val() != "") {
//        $.ajax({
//            //  async: false,
//            method: "POST",
//            url: "/Admin/INV_Almacenes/ObtenerFolioConsecutivo",
//            //contentType: "application/json; charset=utf-8",
//            data: { id_sucursal: $('#Almacen_Id_Sucursal').val() },
//            dataType: "json",
//            beforeSend: function () {
//            },
//            success: function (data) {
//                $('#txtId').val(data.folio);
//            }, error: function (xhr, ajaxOptions, thrownError) {
//                throw new Error('Error al cargar...');
//            }, complete: function () {
//            }
//        });
//    } else {
//        toastr.error('Es necesario seleccionar una sucursal');
//        $('#txtId').val("");

//    };
   

//});

function CargarDataTable() {
    dataTable = $('#tblAlmacenes').DataTable({
        "ajax": {
            "url": "/Cliente/INV_Almacenes/GetAll",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            { "data": "id", "width": "5%" },
            { "data": "descripcion", "width": "20%" },
            //{
            //    "width": "25%", "render": function (data, type, row) {
            //        return row.calle + ' ' + row.numero_Exterior
            //    }
            //},
            { "data": "sucursal.descripcion", "width": "10%" },
            {
                "data": "activo", "width": "10%", "render": function (data, type, row) {
                    if (data == true) {
                        //return `<p class="m-0 text-center"><span class="badge badge-primary">Activo</span></p>`;
                        return `<div class="switch">
                                <div class="onoffswitch">
                                    <input type="checkbox" checked class="onoffswitch-checkbox cb_activo" id="cb_activo_${row.id}">
                                    <label class="onoffswitch-label" for="cb_activo_${row.id}">
                                        <span class="onoffswitch-inner"></span>
                                        <span class="onoffswitch-switch"></span>
                                    </label>
                                </div>
                            </div>`;
                    } else {
                        //return `<p class="m-0 text-center"><span class="badge badge-danger">Inactivo</span></p>`;
                        return `<div class="switch">
                                <div class="onoffswitch">
                                    <input type="checkbox"  class="onoffswitch-checkbox cb_activo" id="cb_activo_${row.id}">
                                    <label class="onoffswitch-label" for="cb_activo_${row.id}">
                                        <span class="onoffswitch-inner"></span>
                                        <span class="onoffswitch-switch"></span>
                                    </label>
                                </div>
                            </div>`;
                    }

                }
            },
            {
                "data": "id", "render": function (data) {
                    return `<div class="text-center">
                            <p class="mb-0">
                                <a href='/Cliente/INV_Almacenes/Edit/${data}' class="btn btn-success btn-circle text-white" ><i class="fa fa-edit btnEditar"></i></a>
                                <a onclick=Delete('/Cliente/INV_Almacenes/Delete/${data}') class="btn btn-danger btn-circle text-white"><i class="fa fa-trash btnEliminar"></i></a>
                            </p>

                            `;
                }, "width": "10%"
            }
        ],
        "language": {
            "decimal": "",
            "emptyTable": "No hay registros",
            "info": "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
            "infoEmpty": "Mostrando 0 to 0 of 0 Entradas",
            "infoFiltered": "(Filtrado de _MAX_ total entradas)",
            "infoPostFix": "",
            "thousands": ",",
            "lengthMenu": "Mostrar _MENU_ Entradas",
            "loadingRecords": "Cargando...",
            "processing": "Procesando...",
            "search": "Buscar:",
            "zeroRecords": "Sin resultados encontrados",
            "paginate": {
                "first": "Primero",
                "last": "Ultimo",
                "next": "Siguiente",
                "previous": "Anterior"
            }
        },
        "width": "100%",
        dom: '<"html5buttons"B>lTfgitp',
        buttons: [
            { extend: 'copy', text: "COPIAR", },
            {
                extend: 'csv', title: _doc, exportOptions: {
                    columns: [0, 1, 2]
                }
            },
            {
                extend: 'excel', title: _doc, text: "EXCEL", exportOptions: {
                    columns: [0, 1, 2]
                }
            },
            {
                extend: 'pdf', title: _doc,
                //text: '<div class="image"><img src="../admin001/img/angular_logo.png" alt="" /></div>',
                text: "PDF",
                exportOptions: {
                    columns: [0, 1, 2]
                },
                customize: function (doc) {

                    doc.content.splice(0, 1, {
                        text: [{
                            text: $('.lblSistemaRZ').text() + ' \n',
                            bold: true,
                            fontSize: 11
                        }, {
                            text: 'Listado de ' + _doc + ' \n',
                            bold: true,
                            fontSize: 16
                        },
                        {
                            text: ' Fecha: ' + moment().format() + ' \n',
                            bold: true,
                            fontSize: 8,
                            alignment: 'left'
                        }
                        ],
                        margin: [0, 0, 0, 12],
                        alignment: 'center'
                    });




                    var cols = [];
                    cols[0] = { text: 'Left part', alignment: 'left', margin: [20] };
                    cols[1] = { text: 'Right part', alignment: 'right', margin: [0, 0, 20] };
                    var objFooter = {};
                    objFooter['alignment'] = 'center';
                    objFooter['columns'] = cols;
                    doc["footer"] = objFooter;

                    doc.defaultStyle.fontSize = 8; //2, 3, 4,etc
                    doc.styles.tableHeader.fontSize = 10; //2, 3, 4, etc
                    //doc.content[1].table.widths = ['10%', '25%', '25%','15%', '15%', '10%'];
                    doc.content[1].table.widths = Array(doc.content[1].table.body[0].length + 1).join('*').split('');
                    //doc.content.splice(1, 0, {
                    //    margin: [0, 0, 0, 12],
                    //    alignment: 'left',
                    //    image: 'data:image/png;base64,'

                    //});


                }
            },

            {
                extend: 'print', title: "Listado de " + _doc, text: "IMPRIMIR",
                exportOptions: {
                    columns: [0, 1, 2]
                },
                customize: function (doc) {
                    $(doc.document.body).addClass('white-bg');
                    //$(win.document.body).css('font-size', '10px');                 
                    //$(win.document.body).find('table')
                    //    .addClass('compact')                       
                    //    .css('font-size', 'inherit');
                    var now = new Date();
                    var jsDate = now.getDate() + '-' + (now.getMonth() + 1) + '-' + now.getFullYear();
                    $(doc.document.body)
                        .prepend('<div style="text-align:center;margin-top:20px;"><h2>' + $('.lblSistemaRZ').text() + '<h2></div>')
                    //.prepend('<div style="text-align:center;margin-top:30px;"><h2>RFC<h2></div>')
                    //.prepend('<img style="position:absolute; top:10; left:0;width:50" src=' + $logo + '>')
                    //.prepend('<div style="position:absolute; top:10; right:0;">Listado de '+ _doc +'</div>')
                    //.prepend('<div style="position:absolute; bottom:20; left:0;">Fecha: ' + jsDate.toString() + '</div>')
                    //.prepend('<div style="position:absolute; top:10; left:50;font-size:24px;">Razon Social:</div>')
                    //.prepend('<div style="position:absolute; top:30; left:450;font-size:20px;margin-botton:50px"></div>')
                    //  .prepend('<div style="position:absolute; bottom:20; left:100;">Pagina '+page.toString()+' of '+pages.toString()+'</div>');

                    $(doc.document.body).find('table')
                        .removeClass('dataTable')
                        .css('font-size', '12px')
                        .css('margin-top', '65px')
                        .css('margin-bottom', '60px')
                    $(doc.document.body).find('th').each(function (index) {
                        $(this).css('font-size', '18px');
                        $(this).css('color', '#fff');
                        $(this).css('background-color', 'blue');
                    });



                }
            },

        ],
        "initComplete": function (settings, json) {
            CargarTooltip();
        }


    });


    ClicTabla();
}

function Delete(url) {
    swal({
        title: "Está seguro de borrar?",
        text: "Este contenido no se puede recuperar",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#DD6B55",
        confirmButtonText: "Si, borrar!",
        closeOnConfirm: true
    }, function () {
        $.ajax({
            type: 'Delete',
            url: url,
            success: function (data) {
                if (data.success) {
                    toastr.success(data.message);
                    dataTable.ajax.reload();
                } else {
                    toastr.error(data.message);
                }
            }

        })

    }
    );
}


function ClicTabla() {
    $("#tblAlmacenes tbody").on("click", ".cb_activo", function (e) {

        var row = $('#tblAlmacenes').DataTable().row($(this).parents('tr')).data();

        $.ajax({
            //  async: false,
            type: "POST",
            url: "/Cliente/INV_Almacenes/CambiaEstatus",
            data: { id: row.id },
            dataType: "json",
            beforeSend: function () {
            },
            success: function (data) {
                if (data.success == false) {
                    swal("Error!", data.message, "warning");
                }
            }, error: function (xhr, ajaxOptions, thrownError) {
                swal("Error!", "", "warning");
            }, complete: function () {
            }
        });

    });

}

