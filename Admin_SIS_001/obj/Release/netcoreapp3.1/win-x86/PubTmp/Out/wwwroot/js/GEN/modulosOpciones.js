...
function cargarTooltip() {
    ...
    new jBox('Tooltip', { attach: '#btnAdd', title: "Agregar Nuevo(a) " + _doc });
    new jBox('Tooltip', { attach: '.btnEditar', title: "Editar " + _doc });
    new jBox('Tooltip', { attach: '.btnEliminar', title: "Eliminar " + _doc });
    //new jBox('Tooltip', { attach: '#btnGuardar', title: "Agregar Nuevo(a) " + doc });
...
$(document).on('change', '#txtId', function () {
    ...
    $.ajax({
        //  async: false,
        method: "POST",
        url: "/Admin/GEN_ModulosOpciones/ExisteClave",
        ...
        success: function (data) {
            if (data.existe == true) {
                $('#txtId').val(""); 
                swal("Admin SIS!", "Esa clave ya existe.", "warning")
            }
        }, error: function (xhr, ajaxOptions, thrownError) {
            swal("Admin SIS!", "Orrio un error", "warning")
        }, complete: function () {
        }
    });
...
function cargarDataTable() {
    dataTable = $('#tblModulosOpciones').DataTable({
        "ajax": {
            "url": "/Admin/GEN_ModulosOpciones/GetAll",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            ...
            { "data": "modulos.descripcion", "width": "20%" },   
            { "data": "tipo", "width": "10%" },
            {
            ...
            },
            {
                "data": "id", "render": function (data, type, row) {
                    var cveCompuesta = row.id + '|' + row.modulos.id;
                    return `<div class="text-center">
                            <p class="mb-0">
                                <a href='/Admin/GEN_ModulosOpciones/Edit/${cveCompuesta}' class="btn btn-success btn-circle text-white" ><i class="fa fa-edit btnEditar"></i></a>
                                <a onclick=Delete('/Admin/GEN_ModulosOpciones/Delete/${cveCompuesta}') class="btn btn-danger btn-circle text-white"><i class="fa fa-trash btnEliminar"></i></a>
                            </p>
...
function ClicTabla() {
    $("#tblModulosOpciones tbody").on("click", ".cb_activo", function (e) {

        var row = $('#tblModulosOpciones').DataTable().row($(this).parents('tr')).data();
       
            $.ajax({
                //  async: false,
                type: "POST",
                url: "/Admin/GEN_ModulosOpciones/CambiaEstatus",
                data: { id: row.id + "|" + row.id_Modulo },
                dataType: "json",
                beforeSend: function () {
                },
                success: function (data) {
