...
namespace Admin_SIS_001.Areas.Admin.Controllers
{
    ...
    public class INV_ProductosController : Controller
    {
        private readonly IContenedorTrabajo _contenedorTrabajo;
        private IHttpContextAccessor _httpContextAccessor;

        public INV_ProductosController(IContenedorTrabajo contenedorTrabajo, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public IActionResult Create()
        {
            ...
            VM_INV_PRODUCTOS _vm = new VM_INV_PRODUCTOS()
            {
                Producto = new Tbl_INV_PRODUCTOS(),
                //ListaImpuestos = new List<Tbl_INV_PRODUCTOS_IMPUESTOS>(),
                ListaImpuestos = _contenedorTrabajo.Producto_impuesto.GetAll(a => a.Id_Dist == id_dist).ToList(),
                ListaLineas = _contenedorTrabajo.Linea.GetListaLineas(id_dist),
                ListaSubLineas = _contenedorTrabajo.SubLinea.GetListaSubLineas(id_dist, 0),
                ListaMarcas = _contenedorTrabajo.Marca.GetListaMarcas(id_dist)
            };

            int folio = 0;

            folio = _contenedorTrabajo.Producto.GetAll(a => a.Id_Dist == id_dist).Count();
            ViewBag.folio = folio += 1;
            return View(_vm);
        }
        ...
        public IActionResult Create(VM_INV_PRODUCTOS vm_producto)
        {
            var id_dist = Decimal.Parse(HttpContext.Session.GetString("id_dist"));
            if (ModelState.IsValid)
            {

                vm_producto.Producto.Id_Dist = id_dist;
                _contenedorTrabajo.Producto.Add(vm_producto.Producto);
                _contenedorTrabajo.Save();
                return RedirectToAction(nameof(Index));
            }
            return View(vm_producto);
        }
        ...
        {
            var id_dist = Decimal.Parse(HttpContext.Session.GetString("id_dist"));
            return Json(new { data = _contenedorTrabajo.Producto.GetAll(a => a.Id_Dist == id_dist) });
        }
