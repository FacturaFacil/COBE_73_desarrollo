...
$(document).on('change', '#SubLinea_Id_Linea', function () {
    if ($('#SubLinea_Id_Linea').val() != "") {
        $.ajax({
            //  async: false,
            method: "POST",
            url: "/Cliente/INV_SubLineas/ObtenerFolioConsecutivo",
            data: { id_linea: $('#SubLinea_Id_Linea').val() },
            dataType: "json",
            beforeSend: function () {
            },
            success: function (data) {
                $('#SubLinea_Id').val(data.folio);
        ...
        toastr.error('Es necesario seleccionar una linea');
        $('#txtId').val("");
...
$(document).on('change', '#SubLinea_Id', function () {
    if ($('#Linea_Id_Linea').val() != "") {
        $.ajax({
            //  async: false,
            method: "POST",
            url: "/Cliente/INV_Sublineas/ExisteFolio",
            //contentType: "application/json; charset=utf-8",
            //data: { param: $('#txtId').val() + "|" + $('#Almacen_Id_Sucursal').val() },
            data: { param: $('#SubLinea_Id_Linea').val() + "|" + $('#SubLinea_Id').val() },
            dataType: "json",
            beforeSend: function () {
            },
            success: function (data) {
                if (data.existe == true) {
                    $('#SubLinea_Id').val(""); $('#SubLinea_Id').val(data.folio);
...
function CargarDataTable() {
    dataTable = $('#tblSubLineas').DataTable({
        "ajax": {
            "url": "/Cliente/INV_SubLineas/GetAll",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            { "data": "lineas.descripcion", "width": "10%" },
            {
                "data": "id", "width": "5%"
                //"data": "id", "width": "5%", "render": function (data, type, row) {
                //    return row.id_Linea + "|" + row.id;
                //}
            },
            { "data": "descripcion", "width": "20%" },
            {
                "data": "activo", "width": "10%", "render": function (data, type, row) {
                    if (data == true) {
                        //var id = row.id_Linea + "|" + row.id;
                        return `<p class="m-0 text-center"><span class="badge badge-success">Activo</span></p>`;
            ...
            },
            {
                "data": "id", "render": function (data, type, row) {
                    var id = row.id_Linea + "|" + row.id;
                    //var id_linea = row.id_Linea
                    return `<div class="text-center">
                            <p class="mb-0">
                                <a href='/Cliente/INV_SubLineas/Edit/${id}' class="btn btn-success btn-circle text-white" ><i class="fa fa-edit btnEditar"></i></a>
                                <a onclick=Delete('/Cliente/INV_SubLineas/Delete/${id}') class="btn btn-danger btn-circle text-white"><i class="fa fa-trash btnEliminar"></i></a>
                            </p>
