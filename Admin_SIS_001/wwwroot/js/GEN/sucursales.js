...
function cargarDataTable() {
    dataTable = $('#tblSucursales').DataTable({
        "ajax": {
            "url": "/Cliente/GEN_Sucursales/GetAll",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            ...
            },
            {
                "data": "id", "render": function (data) {
                    return `<div class="text-center">
                            <p class="mb-0">
                                <a href='/Cliente/GEN_Sucursales/Edit/${data}' class="btn btn-success btn-circle text-white" ><i class="fa fa-edit btnEditar"></i></a>
                                <a onclick=Delete('/Cliente/GEN_Sucursales/Delete/${data}') class="btn btn-danger btn-circle text-white"><i class="fa fa-trash btnEliminar"></i></a>
                            </p>
...
function ClicTabla() {
    $("#tblSucursales tbody").on("click", ".cb_activo", function (e) {

        var row = $('#tblSucursales').DataTable().row($(this).parents('tr')).data();
       
            $.ajax({
                //  async: false,
                type: "POST",
                url: "/Cliente/GEN_Sucursales/CambiaEstatus",
                data: { id: row.id },
                dataType: "json",
                beforeSend: function () {
                },
...
//Eventos
$(document).on('change', '#cbActivo', function () {

    if ($('#cbActivo').is(':checked')) {

        new jBox('Tooltip', { attach: '.onoffswitch-label', title: "Activo(a) " + _doc });
    } else {
        new jBox('Tooltip', { attach: '.onoffswitch-label', title: "Inactivo(a) " + _doc });
    }
...
$(document).on('change', '#txtId', function () {
    ...
    $.ajax({
        //  async: false,
        method: "POST",
        url: "/Cliente/GEN_Sucursales/ExisteFolio",
